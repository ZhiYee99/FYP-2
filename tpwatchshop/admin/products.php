...
$statement = $pdo->prepare('SELECT p.*, GROUP_CONCAT(pi.img) AS imgs FROM products p LEFT JOIN products_images pi ON p.id = pi.product_id GROUP BY p.id ORDER BY p.date_added ASC');
$statement->execute();
$products = $statement->fetchAll(PDO::FETCH_ASSOC);
?>
<?=adminHeader_template('Products', 'products')?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="details" onclick="location.href='index.php?page=product&id=<?=$product['id']?>'">
                    ...
                    <td>
                        <?php if ($product['inactive']==0){echo "Active";}
                         elseif($product['inactive']==1){echo "Inactive";};?>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
