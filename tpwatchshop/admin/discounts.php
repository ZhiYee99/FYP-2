...
$statement = $pdo->prepare('SELECT d.*, GROUP_CONCAT(DISTINCT p.name) product_names, GROUP_CONCAT(DISTINCT c.name) category_names FROM discounts d LEFT JOIN products p ON FIND_IN_SET(p.id, d.product_ids) LEFT JOIN categories c ON FIND_IN_SET(c.id, d.category_ids) GROUP BY d.id');
$statement->execute();
$discounts = $statement->fetchAll(PDO::FETCH_ASSOC);
date_default_timezone_set("Asia/Kuala_Lumpur");
$current_date = strtotime((new DateTime())->format('Y-m-d H:i:s'));
?>
<?=adminHeader_template('Discounts', 'discounts')?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="details" onclick="location.href='index.php?page=discount&id=<?=$discount['id']?>'">
                    ...
                    <td><?=$current_date >= strtotime($discount['start_date']) && $current_date <= strtotime($discount['end_date']) ? 'Active' : 'Inactive'?></td>
